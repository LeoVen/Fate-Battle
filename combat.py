...
def combatMode(ally, enemy, player):
	while ally.TURN > 0:
		clear()
			...
			while True:
				...
				if att == '0':
					...
					if ally.ATTACK == 0:
						...
						attacksDisabled(ally.TURN)
					else:
						error()
						print("Attack error. Attack Variable < 0")
						enterToContinue()
				elif att == 'm' or att == 'M':
					if ally.ATTACK == 0:
						#Attack available
...
def skillAttack(ally, enemy, sk):
	...
	if sk.DMG > 0 or sk.MAG > 0:
		...
		if pas:
			...
			elif enemy.DEFENSE > 0:
				#enemy's defense is off
				#If armor is higher than 100% then no damage is taken so that the enemy hp doesn't get increased
				if enemy.PD <= 100:
					enemy.DAMAGE += sk.DMG * mod
				if enemy.MD <= 100:
					enemy.DAMAGE += sk.MAG * mod
			else:
				error()
				print("Defense error. Defense variable < 0")
				enterToContinue()
		else:
			skillMissed()
